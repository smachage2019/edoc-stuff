...
df = pd.read_excel('Values4.xlsx', sheet_name='Sheet1')
...
clust_list = dfstg.StgClust.unique()
clust_list.sort()
print(clust_list)
clust = clust_list[0]
print(clust)
...
dfclust = dfstg.loc[dfstg['StgClust'] == clust]
dfclust.head()

x_data = dfclust['Elapsed_Time']
print(x_data)
y_data = dfclust['Distance From Perfs']
print(y_data)
...
#out.to_csv(r'J:\Double Eagle\James\svm\reg\elapsed\out.csv', index = False)
out.to_csv(well+"-Stage-"+stage+"-Clust-"+clst+" Curve.csv", sep=',', index=False)


# Check
x_data = dfclust["Easting ft (Abs)"]
print(x_data)
y_data = dfclust['Northing ft (Abs)']
print(y_data)
plt.plot(x_data, y_data, '.', alpha=0.2, label = 'Stage data')
plt.plot(dfclust['pEast'], dfclust['pNorth'], '.', alpha=0.2, label = 'Stage data')
